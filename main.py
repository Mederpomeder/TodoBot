...
class TodoBot:
    def __init__(self):
        self.bot = telebot.TeleBot('6206188066:AAGIxGdVdipnw52H5DD419nB_GER-F3owQo')
        self.tasks = {}
    ...
    def read_todos(self, chat_id):
        if chat_id in self.tasks and self.tasks[chat_id]:
            task_list = '\n'.join(f"ID: {task['id']}, Текст: {task['text']}, Выполнено: {task['completed']}" for task in
                                  self.tasks[chat_id])
            self.bot.send_message(chat_id, f'Задания:\n{task_list}', reply_markup=self.show_menu())
        else:
            self.bot.send_message(chat_id, 'Ваш список заданий пуст!', reply_markup=self.show_menu())
    ...
    def retrieve_todo_step(self, message):
        ...
        if chat_id in self.tasks:
            ...
            if task:
                ...
                self.bot.send_message(chat_id, 'Задание не найдено', reply_markup=self.show_menu())
        else:
            self.bot.send_message(chat_id, 'Задание не найдено', reply_markup=self.show_menu())

    def create_todo(self, chat_id):
        self.bot.send_message(chat_id, 'Введите задание:')
    ...
    def create_todo_step(self, message):
        ...
        self.bot.send_message(chat_id, 'Задание успешно добавлено!\nВыберите следующее действие:',
                              reply_markup=self.show_menu())
    ...
    def update_todo_step(self, message):
        ...
        if chat_id in self.tasks:
            task = next((task for task in self.tasks[chat_id] if task['id'] == task_id), None)
            if task:
                self.bot.send_message(chat_id, 'Введите новое название задания:')
    ...
    def update_todo_text_step(self, message, task):
        ...
        task_text = message.text.strip()
        task['text'] = task_text
        self.bot.send_message(chat_id, 'Обновлено успешно!\nВыберите следующее действие:',
                              reply_markup=self.show_menu())
    ...
    def delete_todo_step(self, message):
        ...
        if chat_id in self.tasks:
            ...
            if task:
                ...
                self.bot.send_message(chat_id, 'Удалено успешно!\nВыберите следующее действие:',
                                      reply_markup=self.show_menu())
            else:
                self.bot.send_message(chat_id, 'Задание не найдено', reply_markup=self.show_menu())
        else:
    ...
    def completed_todo_step(self, message):
        ...
        if chat_id in self.tasks:
            ...
            if task:
                task['completed'] = True
                self.bot.send_message(chat_id, 'Задание выполнена!', reply_markup=self.show_menu())
            else:
                self.bot.send_message(chat_id, 'Задание не найдено', reply_markup=self.show_menu())
        else:
            self.bot.send_message(chat_id, 'Задание не найдено', reply_markup=self.show_menu())
